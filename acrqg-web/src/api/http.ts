...
async function refreshAccessToken(): Promise<string> {
    ...
    refreshingPromise = http
        .post<ApiResponse<RefreshResultDTO>>(
            '/auth/refresh',
            { refreshToken },
            { skipAuth: true, skipErrorMessage: true } as ExtendedRequestConfig,
        )
        .then((data) => {
            // 响应拦截器已解包：返回的 data 即 RefreshResultDTO
            const result = data as unknown as RefreshResultDTO
            auth.setAccessToken({ accessToken: result.accessToken, expiresIn: result.expiresIn })
